...
class ChampionData extends React.Component {
    constructor(){
        super();
        this.state = {cards: [], AllData: [], RequestedData: []}
    }
    ...
    render(){
        ...
        let RequestedData = this.state.RequestedData;
            ...
            const LoadAllAPI = async () => {
                ...
                for (const [x, card] of Object.entries(cards)) {
                    ...
                    d.querySelector(`#champion-card-${x}`).addEventListener('click', async () => {
                        // Fetch the json of the champion the user clicked on, and pass it to the Champion Panel component, to assign the data to the DOM.
                        if (RequestedData.length >= 1) {
                            ...
                            const json = await response.json();
                            Object.keys(json.data).forEach(key => {
                                RequestedData[x] = json.data[key];
                            });
                            this.setState({RequestedData: RequestedData});
                        }
                        ...
                        const { name, id, title, key, skins, spell } = champion;
        ...
        return([
            ...
            <ChampionDetailPanel IndividualChamp = {this.state.RequestedData} />
        ])
    }
