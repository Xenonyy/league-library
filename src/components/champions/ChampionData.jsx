  ...
  render() {
    ...
    const LoadAllAPI = async () => {
      for (const [x, card] of Object.entries(cards)) {
        ...
        d.querySelector(`#champion-card-${x}`)?.addEventListener(
          'click',
          () => {
            ...
            const { name, id, title, key, skins, blurb, lore, spell } =
              champion;
            ...
            d.getElementById('champion-detail-description-full').innerText =
              lore;
            for (const [key, spell] of Object.entries(spells)) {
              d.querySelector(
                `#champion-detail-abilities-${key}-name`,
            ...
            for (const [x, ability] of Object.entries(spells)) {
              d.querySelector(
                ...
                ).innerText = `${x.toUpperCase()} - ${ability.name}`;
    ...
    LoadAllAPI();
    // }
    return <ChampionMainPage cards={this.state.cards} />;
  }
}
